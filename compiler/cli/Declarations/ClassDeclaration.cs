...
{
    public static bool TryParse(Declaration declaration, out ClassDeclaration classDeclaration) {
...
public record ClassDeclarationHead(ClassSignature Me, ClassSignature[] BaseClasses, Tag[] Tags) {
    public static bool TryParse(BlockHead head, out ClassDeclarationHead result) {
        ...
        result = new(
            me,
            bases,
            remaining.Select(Tag.Parse).ToArray()
        );  
